...
export const Row: React.FC<Props>=({row})=>
{
    ...
    {
        return Math.min(window.innerHeight, window.innerWidth)/dimensions-2
    }
    function getStyle(cellValue:number):CSSProperties
    {
        const size = getSize()
        return{width:size, height:size, backgroundColor: cellValue?"black":"white", border:"solid 1px gray"}
    }
    function getDivs()
    {
        return row.map((value,index)=><div style={getStyle(value)} key={index}></div> )
    }
    return <div style={{display:"flex", flexDirection: "row"}}>{getDivs()}</div>
}